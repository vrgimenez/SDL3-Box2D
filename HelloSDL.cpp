...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[])
{
    ...
    if (!SDL_CreateWindowAndRenderer("Hello Renderer Textures", WINDOW_WIDTH, WINDOW_HEIGHT, SDL_WINDOW_RESIZABLE, &window, &renderer)) {
    ...
    texture_width = surface->w / 2;
    texture_height = surface->h / 2;
...
SDL_AppResult SDL_AppIterate(void *appstate)
{
    ...
    /* top left */
    dst_rect.x = ((float) (texture_width * 1/2)) + (100.0f * scale);
    dst_rect.y = 0.0f;
    ...
    /* bottom right. */
    dst_rect.x = ((float) (WINDOW_WIDTH - texture_width * 3/2)) - (100.0f * scale);
